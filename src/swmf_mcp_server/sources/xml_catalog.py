...
def parse_param_xml_file(path: Path, component: str | None) -> list[CommandMetadata]:
    ...
    for match in _COMMAND_OPEN_RE.finditer(text):
        ...
        commands.append(
            CommandMetadata(
                ...
                ranges=ranges,
                source_kind="component PARAM.XML" if component else "PARAM.XML",
                source_path=str(path),
                authority="authoritative",
            )
        )

    # Also collect self-closing command tags.
    ...
    # Fallback extraction for imperfect XML files where command tags are sparse.
